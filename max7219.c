...
//
static void maxSendSequence(uint8_t *pSequence, uint8_t len)
{
   if (len % 2 == 1 || len == 0)
   {
      printf("*** Error: send of length: %d\n", len);
      return;
   }
   printf("sendSequence of %d: ", len);
  ...
  // controllers in a single transaction. When the CS line rises, the data will be latched
   ...
   {
      while (spi_status_read() != 1) { puts("Busy...\n"); }
...
{
  printf("Init: Clock Divider: %d\n", spi_clk_divider_read());
